...
public class AccountRestService {
  ...
  public TokenResponse login(LoginRequest request) {
    ...
    final boolean ok = BcryptUtil.matches(request.password(), account.password());
    if (!ok) {
      throw HttpProblemException.unauthorized("Username or password was not correct");
    }
  ...
  public TokenResponse refresh(RefreshRequest request) {
    ...
    if (!refreshToken.matchesHash(request.refreshToken())) {
      throw HttpProblemException.unauthorized("Refresh token was not correct");
    }
    if (refreshToken.hasExpired()) {
      throw HttpProblemException.unauthorized("Refresh token has expired");
    }
    final JwtToken jwtToken = accountService.issueJwtToken(account);
  ...
  public TokenResponse signup(SignupRequest request) {
    ...
    if (optionalAccount.isPresent()) {
      throw HttpProblemException.unauthorized("Username already exists");
    }
